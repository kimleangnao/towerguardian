$(function(){
	...
	let maxBossHp = localStorage.getItem("ElevenbossHp");
	let bossAttack = 13003 ;
	let currentFloor = localStorage.getItem("Floor");

	//console.log(userHp + " - " + userMana + " - " + userExp + " - " + currentFloor);
	let currentBossName = localStorage.getItem("ElevenbossName");
	let currentBossHp = localStorage.getItem("ElevenbossHp");
	...
	$("#bossPic").on('click', function(){
		...
		if (currentBossHp <= 0) {
			...
			document.getElementById("victoryMess").innerHTML = "You have defeat the last guardian."
		...
		$("#confirm").on('click', function(){
			//console.log("dog");
			let newExp = parseInt(userExp) + 9999999;
			...
			$("#nextFloor").on("click", function(){
				//window.location.replace("floorFinalBoss.html");
			});
		});
	...
	$("#previousFloor").on('click', function(){
		window.location.replace("floorTen.html");
	});
