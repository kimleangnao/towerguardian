$(function(){
	...
	let maxBossHp = localStorage.getItem("FourbossHp");
	let bossAttack = 210;
	let currentFloor = localStorage.getItem("Floor");

	//console.log(userHp + " - " + userMana + " - " + userExp + " - " + currentFloor);
	let currentBossName = localStorage.getItem("FourbossName");
	let currentBossHp = localStorage.getItem("FourbossHp");
	...
	if (userHp > 0 ){
		...
		$("#confirm").on('click', function(){
			//console.log("dog");
			let newExp = parseInt(userExp) + 260;
			...
			$("#nextFloor").on("click", function(){
				window.location.replace("floorFive.html");
			});
		});
	...
	$("#previousFloor").on('click', function(){
		window.location.replace("floorThree.html");
	});
